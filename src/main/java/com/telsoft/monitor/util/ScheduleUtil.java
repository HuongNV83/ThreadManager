...
public class ScheduleUtil
{
    ...
    public static int getStatus(Dictionary dic, Date dtExpectedDate, int iExecutionCount)
    {
        ...
        if (strExecutionTime != null && strExecutionTime.length() > 0 && !strExecutionTime.equals("0"))
        {
        ...
        if ((strWeekDay.length() == 0 ||
            ...
            (strMonthDay.length() == 0 ||
             strMonthDay.indexOf(String.valueOf(cld.get(Calendar.DAY_OF_MONTH)) + ",") >= 0) &&
    ...
    public static void changeNextDate(Dictionary dic, boolean bForceChange)
    {
        ...
        if (dt != null)
        {
            DictionaryNode nd = dic.getChild("ExpectedDate");
    ...
    public static Vector scriptToSchedule(Dictionary dic) throws Exception
    {
        ...
        Vector vt = dic.mndRoot.getChildList();
        for (int iIndex = 0; iIndex < vt.size(); iIndex++)
        {
            DictionaryNode nd = (DictionaryNode)vt.elementAt(iIndex);
            vtSchedule.add(new Dictionary(new ByteArrayInputStream(nd.getValue().getBytes())));
        }
        return vtSchedule;
    }
    ////////////////////////////////////////////////////////
